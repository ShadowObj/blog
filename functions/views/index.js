export async function onRequestGet(context) {
    ...
    context.waitUntil(new Promise(() => ps.all()));

    let total = Object.values(originData).reduce((acr, cur) => {return acr + cur;});
    if ((total%100) == 0) {
        context.waitUntil(new Promise(() => context.env.BLOG_DB.prepare(`ALTER TABLE views_total ADD '${total}' INTEGER NOT NULL DEFAULT ${Date.now()}`).all()));
    }
